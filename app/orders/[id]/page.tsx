...
export default async function OrderPage({
    ...
    return (
      <>
        ...
        <main className="mx-auto max-w-2xl px-5 py-20">
          <h1 className="display text-5xl">Order not available</h1>
          <p className="mt-4 text-black/60">
            This order can only be viewed from the device where it was placed.
          </p>
          <Link
            href="/"
  ...
  const currentIndex = statusIndex(order.fulfillment_status);
        ...
        <section className="mt-5 border border-black bg-white p-6">
          ...
          <div className="mt-5 grid gap-4 text-sm sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="mt-1 font-bold">{order.customer_phone}</p>
            </div>
          ...
          {order.delivery_note && (
            <div className="mt-5 border-t border-black/10 pt-5">
              <p className="text-xs text-black/45">Delivery note</p>
              <p className="mt-1 text-sm">{order.delivery_note}</p>
            </div>
          )}
        </section>
